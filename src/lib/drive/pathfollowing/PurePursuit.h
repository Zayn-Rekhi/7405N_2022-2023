...
class PurePursuit {
   ...
   public:
    ...
    Point getLookAhead(int prev_index, int next_index, Point curPos);
    Point calcActLookAhead(Point curPos, int closestPointIndex);
    int getClosestIndex(Point curPos);
    int getClosestIndexNeon(PPoint curPos);
    double getCurve(Point curPos, Point next);
